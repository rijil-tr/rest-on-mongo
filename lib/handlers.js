...
async function handleGetMany(req, res, next) {
  try {
    ...
    const filter = req.query.__filter ? EJSON.parse(req.query.__filter) : q.filter;
    if(q.include_count) {
      const count = await req.collection.find(filter).count();
      res.set('X-Total-Count', count);
    }
    const cursor = req.collection.find(filter).sort(q.sort).skip(q.skip).limit(q.limit).project(q.projection);
